...
resource "azurerm_network_security_group" "nsg-node2" {
...
resource "azurerm_network_interface" "vnic-node2" {
  name                = "vnic-node2"
  location            = var.location
  resource_group_name = data.azurerm_resource_group.rg.name
  ...
  ip_configuration {
    ...
    private_ip_address            = "172.59.1.12"
    public_ip_address_id          = azurerm_public_ip.pip-node2.id
  }
...
resource "azurerm_public_ip" "pip-node2" {

  name                = "pip-node2-linux"
  resource_group_name = data.azurerm_resource_group.rg.name
  location            = var.location
...
resource "azurerm_network_interface_security_group_association" "nsg-peer" {
  network_interface_id      = azurerm_network_interface.vnic-node2.id
  network_security_group_id = azurerm_network_security_group.nsg-node2.id
}
...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  network_interface_ids = [
    azurerm_network_interface.vnic-node2.id,
  ]
